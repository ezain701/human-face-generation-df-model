...
Diffusion models provide such an alternative. The core idea, first proposed by Sohl-Dickstein et al. \cite{b4} and drawn from nonequilibrium thermodynamics, is to define a forward process that gradually corrupts data by adding Gaussian noise over many timesteps, and then to learn a reverse process that removes noise step by step, recovering samples from the data distribution.

Ho et al. \cite{b3} made this framework practical with Denoising Diffusion Probabilistic Models (DDPMs), and this formulation of DDPM forms the basis of the model implemented in this project. Song et al. \cite{b2} unified discrete-time denoising diffusion with continuous-time score-based generative modeling under a single stochastic differential equation framework, showing that both approaches can be viewed as learning the score function of a data distribution perturbed by noise at varying scales. 

Shortly afterwards, Dhariwal and Nichol \cite{b1} demonstrated empirically that diffusion models outperform GANs on standard image synthesis benchmarks across multiple resolutions, through a combination of architectural improvements, a learned reverse-process variance, and classifier-based guidance for conditional generation.

Prompt conditioning extends diffusion models from unconditional sampling to user-controllable generation. Classifier-free guidance, introduced by Ho and Salimans \cite{b12}, trains a conditional model with occasional conditioning dropout so that both conditional and unconditional predictions can be combined at sampling time. This allows a guidance scale to control how strongly the generated image follows the prompt without requiring a separate classifier. In text-to-image systems, CLIP-style text encoders \cite{b13} are commonly used to convert natural-language prompts into semantic embeddings, and latent diffusion models \cite{b14} demonstrated the effectiveness of combining text conditioning with diffusion-based image synthesis.

Subsequent work has identified several refinements to the original DDPM formulation that improve sample quality at modest implementation cost. Nichol and Dhariwal \cite{b8} introduced a cosine noise schedule in which the cumulative signal-preservation factor decays as a shifted squared cosine. The cosine schedule destroys image information more gradually across timesteps, avoiding the rapid saturation observed at late timesteps under linear scheduling, and yields improved FID at equivalent training cost. 
...
\bibitem{b3} J. Ho, A. Jain, and P. Abbeel, "Denoising diffusion probabilistic models," in Advances in Neural Information Processing Systems, 2020.
...
\bibitem{b10} M. Ott, S. Edunov, D. Grangier, and M. Auli, ``Scaling Neural Machine Translation,'' in Proc. 3rd Conf. Mach. Transl. (WMT), Brussels, Belgium, Oct. 2018, pp. 1--9.
...
\bibitem{b12} J. Ho and T. Salimans, ``Classifier-Free Diffusion Guidance,'' in NeurIPS Workshop on Deep Generative Models and Downstream Applications, 2021.
\bibitem{b13} A. Radford, J. W. Kim, C. Hallacy, A. Ramesh, G. Goh, S. Agarwal, G. Sastry, A. Askell, P. Mishkin, J. Clark, G. Krueger, and I. Sutskever, ``Learning Transferable Visual Models From Natural Language Supervision,'' in Proc. Int. Conf. Mach. Learn. (ICML), pp. 8748--8763, 2021.
\bibitem{b14} R. Rombach, A. Blattmann, D. Lorenz, P. Esser, and B. Ommer, ``High-Resolution Image Synthesis With Latent Diffusion Models,'' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), pp. 10684--10695, 2022.

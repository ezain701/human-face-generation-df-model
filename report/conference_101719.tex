...
\subsection{Effect of Cosine Noise Scheduling}

A cosine noise schedule was also evaluated to investigate whether changing the diffusion noising process could improve sample quality. Unlike cosine learning rate scheduling, which affects the optimizer, cosine noise scheduling changes how noise is added across diffusion timesteps.

The experiment used the same EMA-based configuration as the F-series baseline, with 128$\times$128 resolution, 64 base channels, batch size 1, and 300 epochs. However, replacing the standard linear noise schedule with a cosine noise schedule resulted in a significantly worse FID score of 314.

This indicates that the cosine noise schedule was not effective in the current implementation. The result suggests that the diffusion noise schedule is highly sensitive, and that the original linear schedule was more stable for this setup.
...
F200 EMA & 128px, ch64, EMA, 200 epochs & $\sim$95.30 \\
F250 EMA & 128px, ch64, EMA, 250 epochs & $\sim$76.62 \\
F300 EMA & 128px, ch64, EMA, 300 epochs & $\sim$62.72 \\
...
F300 EMA + cosine noise & 128px, ch64, EMA, cosine noise & 314.10 \\
...
EMA raw check & ch64, 10 epochs (no EMA) & $\sim$234.82 \\
EMA check & ch64, 10 epochs (EMA) & $\sim$173.66 \\
